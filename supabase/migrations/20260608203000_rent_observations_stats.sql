...
CREATE POLICY "Rent observations public read"
  ON public.notice_rent_observations
  FOR SELECT
  USING (true);
...
DROP TRIGGER IF EXISTS notice_rent_observations_updated_at
  ON public.notice_rent_observations;

CREATE TRIGGER notice_rent_observations_updated_at
  BEFORE UPDATE ON public.notice_rent_observations
  FOR EACH ROW
  EXECUTE FUNCTION public.touch_notice_rent_observations_updated_at();
