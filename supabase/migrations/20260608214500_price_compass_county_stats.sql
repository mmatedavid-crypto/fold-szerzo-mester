...
CREATE POLICY "Sale price observations public read"
  ON public.notice_sale_price_observations
  FOR SELECT
  USING (true);
...
DROP TRIGGER IF EXISTS notice_sale_price_observations_updated_at
  ON public.notice_sale_price_observations;

CREATE TRIGGER notice_sale_price_observations_updated_at
  BEFORE UPDATE ON public.notice_sale_price_observations
  FOR EACH ROW
  EXECUTE FUNCTION public.touch_notice_sale_price_observations_updated_at();
...
REVOKE ALL ON FUNCTION public.touch_notice_sale_price_observations_updated_at() FROM anon, authenticated;
