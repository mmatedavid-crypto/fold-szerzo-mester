...
import { AlertTriangle, CheckCircle2, Minus, HelpCircle, Ban, Scale, Sparkles, Copy, ChevronDown } from "lucide-react";
...
export function ResultPanel({
  ...
  const copyProofs = () => {
    ...
    navigator.clipboard.writeText(text).then(() => toast.success("Igazolási lista vágólapra másolva"));
  };
  ...
  return (
    <Card className="p-5 space-y-4 lg:sticky lg:top-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-base font-semibold">Eredmény</h3>
      </div>

      <div className={`rounded-lg border-2 p-4 ${meta.cls}`}>
        <div className="flex items-center gap-2 mb-2">
          {meta.icon}
          <Badge variant="outline" className="border-current font-bold tracking-wide">{meta.badge}</Badge>
        </div>
      ...
      {comparison !== "empty" && (
        <div className="grid grid-cols-2 gap-3">
          <RankCell title="Kifüggesztett bérlő" rank={lesseeStrongestRank?.humanName} ref={lesseeStrongestRank?.legalRef} state={lesseeStrongestRank?.state} />
          <RankCell title="Te" rank={userStrongestRank?.humanName} ref={userStrongestRank?.legalRef} state={userStrongestRank?.state} />
        </div>
      )}
      ...
      <div className="space-y-2 pt-2">
        ...
        {comparison === "same_rank" && (
          <>
            <Button className="w-full" onClick={onAccept}>Elfogadó nyilatkozat előkészítése</Button>
            <p className="text-xs text-amber-700">Azonos ranghely esetén nem biztos, hogy átveheted a bérlő helyét. Jogi ellenőrzés javasolt.</p>
          </>
        )}
        {comparison === "lessee_unknown" && (
          <>
            <Button className="w-full" onClick={onAccept}>Elfogadó nyilatkozat előkészítése</Button>
            <p className="text-xs text-amber-700">A bérlő jogcíme nélkül a kimenetel bizonytalan.</p>
          </>
        )}
        ...
        {comparison === "incomplete_special" && (
          <Button className="w-full" variant="outline">Hiányzó feltételek bepipálása</Button>
        )}
        {comparison === "cannot_determine" && (
          <Button className="w-full" variant="outline">Adatok pontosítása</Button>
        )}
      </div>
      ...
      {(userStrongestRank || lesseeStrongestRank) && (
        <Collapsible open={openLegal} onOpenChange={setOpenLegal} className="border-t pt-3">
          <CollapsibleTrigger className="text-xs text-muted-foreground flex items-center gap-1 cursor-pointer">
            <ChevronDown className={`h-3 w-3 transition-transform ${openLegal ? "rotate-180" : ""}`} /> Jogszabályi háttér
          </CollapsibleTrigger>
          <CollapsibleContent className="mt-2 space-y-1 text-xs text-muted-foreground">
            ...
            {lesseeStrongestRank && lesseeStrongestRank.id !== userStrongestRank?.id && <div>• {lesseeStrongestRank.legalRef}</div>}
          </CollapsibleContent>
        </Collapsible>
      )}
...
function RankCell({ title, rank, ref: legalRef, state }: { title: string; rank?: string; ref?: string; state?: "match" | "incomplete" }) {
  return (
    <div className="rounded border p-2.5">
      <div className="text-xs text-muted-foreground">{title}</div>
      <div className="text-sm font-medium mt-1 leading-tight">{rank ?? "—"}</div>
      {state && (
        <Badge variant="outline" className={`mt-1 text-[10px] ${state === "match" ? "text-emerald-700 border-emerald-300" : "text-amber-700 border-amber-300"}`}>
...
const META: Record<LeaseComparisonResult["comparison"], { badge: string; cls: string; icon: ReactElement }> = {
  ...
  lessee_unknown: {
    ...
    icon: <HelpCircle className="h-4 w-4 text-sky-700" />,
  },
  incomplete_special: {
  ...
  exception: {
    badge: "KIVÉTEL LEHET",
    cls: "border-purple-300 bg-purple-50 text-purple-900",
