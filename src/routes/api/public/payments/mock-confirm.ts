  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        // Mark as paid
        await supabaseAdmin.from("payments").update({ status: "paid", paid_at: new Date().toISOString() }).eq("id", paymentId);

        // Grant credit or subscription
        if (payment.product_type === "single") {
          await supabaseAdmin.from("document_credits").insert({
            user_id: payment.user_id, source_type: "single_purchase", status: "available", payment_id: payment.id,
          });
        } else {
          ...
          if (plan) {
            ...
            const quotaEnd = new Date(); quotaEnd.setFullYear(quotaEnd.getFullYear() + 1);
            ...
            await supabaseAdmin.from("subscriptions").insert({
              user_id: payment.user_id, plan_id: plan.id, status: "active",
              current_period_start: start.toISOString(), current_period_end: periodEnd.toISOString(),
              annual_quota: plan.annual_quota, used_quota: 0,
              quota_period_start: start.toISOString(), quota_period_end: quotaEnd.toISOString(),
        ...
        await supabaseAdmin.from("usage_logs").insert({
          user_id: payment.user_id, action: "payment.paid", entity_type: "payment", entity_id: payment.id,
        });
