...
export const subscribeToSettlement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: existing, error: selErr } = await supabaseAdmin
      ...
      .maybeSingle();
    if (selErr) return { ok: false as const, error: "Az értesítő beállítása most nem sikerült." };
    ...
    if (existing) {
      ...
      const { error: updErr } = await supabaseAdmin
        ...
        .eq("id", existing.id);
      if (updErr) return { ok: false as const, error: "Az értesítő megújítása most nem sikerült." };
    ...
    if (insErr) return { ok: false as const, error: "Az értesítő beállítása most nem sikerült." };
...
export const unsubscribeByToken = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: row, error } = await supabaseAdmin
      ...
      .maybeSingle();
    if (error) return { ok: false as const, error: "A leiratkozás most nem sikerült." };
