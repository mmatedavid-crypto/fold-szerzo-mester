...
const PUBLIC_SITE = process.env.PUBLIC_SITE_URL ?? "https://project--37a86164-149b-4ec6-a091-9c93b390c191.lovable.app";

function htmlEscape(s: string): string {
  return s.replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]!));
}

function renderEmail(sub: Subscription, notices: Notice[]): { subject: string; html: string; text: string } {
  ...
  const rows = notices
    .map((n) => {
      ...
      const pubDate = n.publication_date ? new Date(n.publication_date).toLocaleDateString("hu-HU") : "—";
      const link = n.original_detail_url
        ? `<a href="${htmlEscape(n.original_detail_url)}" style="color:#0a6;">Szerződés megtekintése</a>`
        : "—";
      return `<tr>
...
<html><body style="font-family:Arial,sans-serif;background:#fff;color:#222;padding:24px;">
  ...
  const text = `Heti kifüggesztések – ${sub.settlement_clean}\n\n${notices.length} aktuális kifüggesztés.\n\n` +
    notices.map((n) => `- ${n.publication_date ?? ""} ${(n.parcel_numbers ?? []).join(", ")} ${n.notice_type ?? ""} ${n.original_detail_url ?? ""}`).join("\n") +
...
async function sendResendEmail(to: string, subject: string, html: string, text: string): Promise<void> {
  ...
  if (!LOVABLE_API_KEY || !RESEND_API_KEY) throw new Error("Missing LOVABLE_API_KEY or RESEND_API_KEY");
...
export async function sendWeeklyDigest(): Promise<{ sent: number; skipped: number; failed: number; expired: number; errors: string[] }> {
  ...
  if (list.length === 0) return { sent: 0, skipped: 0, failed: 0, expired: expiredCount ?? 0, errors: [] };
  ...
  const { data: allNotices, error: nerr } = await supabaseAdmin
    .from("notices")
    .select("id, subject, settlement, parcel_numbers, notice_type, publication_date, original_detail_url")
  ...
  for (const sub of list) {
    ...
    try {
      ...
      failed++;
      errors.push(`${sub.email}/${sub.settlement_clean}: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
