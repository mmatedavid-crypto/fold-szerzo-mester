...
export const finalizeContract = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: docId, error: rpcErr } = await (supabaseAdmin as any).rpc("finalize_document", {
      ...
      _template_version: `${tpl.version} / ${LEGAL_RULESET_VERSION}`,
      _clause_version: `${tpl.version} / ${LEASE_CLAUSE_VERSION}`,
    ...
    if (rpcErr) {
      ...
      if (rpcErr.message?.includes("no_credit_or_quota")) {
        throw new Error(
          "Nincs elérhető szerződés-kredited vagy aktív előfizetési kereted. Kérjük, indíts fizetést.",
        );
      }
...
export const getDownloadUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: doc, error } = await supabase
      ...
      .select("pdf_file_path, document_number")
    ...
    const { data: signed, error: sErr } = await supabaseAdmin.storage
      .from("contracts")
      .createSignedUrl(doc.pdf_file_path, 300);
...
export const listMyDocuments = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("generated_documents")
      .select(
        "id, document_number, lessor_name, lessee_name, settlement, parcel_numbers, finalized_at, legal_template_version",
      )
      .eq("user_id", userId)
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: subs }, { data: credits }] = await Promise.all([
      ...
      supabase
        ...
        .eq("user_id", userId)
        .eq("status", "available")
        .eq("source_type", "single_purchase"),
    ]);
    ...
    return {
      ...
      subscription: activeSub
        ? {
            used: activeSub.used_quota,
            total: activeSub.annual_quota,
            period_end: activeSub.quota_period_end,
          }
        : null,
    };
  });
