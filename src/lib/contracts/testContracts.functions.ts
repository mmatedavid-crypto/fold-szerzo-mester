 ...
 * feltölti a `contracts` privát storage bucketbe, és aláírt linkekkel egyetlen
 * összefoglaló e-mailt küld a megadott ügyvédnek.
 *
 * NEM megy át a `finalizeContract` ügyvédi-jóváhagyási kapun (mert a teszt
 * pont arra szolgál, hogy az ügyvéd először lássa a kimenetet), és nem
 * fogyaszt kreditet. Csak admin szerepkörrel hívható.
 */
...
function makeDraft(
  id: string,
  partial: Partial<Draft> & Pick<Draft, "lessor_data" | "lessee_data" | "parcels" | "rent" | "term">,
): Draft {
  return {
    id,
...
export const sendTestContractsToLawyer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // Active template + clauses (same path as production finalize)
    ...
    // Merge clause overrides (so the lawyer sees their edits)
    ...
    for (let i = 0; i < SAMPLES.length; i++) {
      ...
      const { data: signed, error: sErr } = await supabaseAdmin.storage
        .from("contracts")
        .createSignedUrl(path, 60 * 60 * 24 * 7); // 7 nap
